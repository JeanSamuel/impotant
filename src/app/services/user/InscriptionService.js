...
class InscriptionService {
	...
	async _registerUser(dataInscription, activity) {
		...
		try {
			...
			const userInfo = await UserService.getUserInfo(account_id, activity);
	...
	_validationCompte(activity) {
		//console.log("profile",activity.state.profile,"addresse",activity.state.addresse,"data",activity.state.data);
		let msg = null;
		try {
			switch (activity.state.page) {
				case 0:
